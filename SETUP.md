...
http://localhost:4000. The lab pages are committed in `lab/`, so they need no
extra step unless you have changed a source in `_lab/`. Behind a slow connection to rubygems.org:
...
| Edit or add a lab page | see [Lab pages](#lab-pages) |
...
`/lab/` lists single-page interactive study notes. Each one is a standalone page,
not a Jekyll layout, so it keeps its own design and scripts.

```
_lab/<slug>/            source — edit here (underscore dir: Jekyll ignores it)
lab/<slug>/index.html   generated by `npm run lab:build` — committed, served as-is
_data/lab.yml           the list on /lab/ and in /zh/'s 交互式学习笔记, in display order
bin/lab-build.mjs       the build
```

A source is either a **Vite package** (`package.json`, partials under `src/`) or
a **single `index.html`**. The generated file is always one self-contained page
with CSS and JS inlined, so jekyll-terser never sees a separate `.js`. It makes
no request off-site: Google Fonts links are stripped, and every page already
declares system fallback fonts. A small "← Zinan Yang · lab" bar is injected
at the top.

| Want to… | Do this |
|---|---|
| Work on a Vite page | `cd _lab/<slug>`, `npm install`, `npm run dev` |
| Work on a single-file page | edit `_lab/<slug>/index.html`, open it in a browser |
| Publish changes | `npm run lab:build` (or `npm run lab:build -- <slug>`), then commit `_lab/` and `lab/` together |
| Add a page | put the source in `_lab/<slug>/`, add an entry with that `slug` to `_data/lab.yml`, build, commit |
| Republish a Vite page to claude.ai | `npm run artifact` inside its folder, as its README describes |

The build **fails** if a page still loads anything off-site, if a local asset
was left un-inlined, if `_data/lab.yml` and `_lab/` disagree about which pages
exist, or if a source or output contains a denylisted phrase (`DENYLIST` in
`bin/lab-build.mjs`: private paths, job-search and archive references,
internal-only markings). The rules behind that list:

- Nothing from a customer project, an employer's internal documents, or a
  personal archive. Cite public sources only: Siemens Industry Online Support
  entries, developer.siemens.com, papers, open-source repos.
- Pages about Siemens products say so. Each carries an "Unofficial notes"
  eyebrow and the line *"Personal study notes — not a Siemens publication"*
  near the top, and the group in `_data/lab.yml` has `unofficial: true`.
- Anything a page was told to keep private stays out. The ROXSIE page, for
  example, had a restricted service-bundle pricing block removed on import.
